...
app.post('/login', async (req, res) => {
  ...
  const token = jwt.sign({ username: user.username, name: user.username }, SECRET, { expiresIn: '150d' });
...
app.post('/task', authMiddleware, async (req, res) => {
  ...
  const message = {
    notification: {
      title: 'Heads Up! New Task Just Dropped ⚡',
      body: `📌 ${task.name} at ${task.time} (${task.week} week)`
    },
    tokens
  };
  ...
  try {
    ...
    if (response.failureCount > 0) {
      ...
      if (failedTokens.length > 0) {
        ...
        console.log(`[FCM CLEANUP] Removed ${failedTokens.length} invalid tokens.`);
      }
    }
  } catch (err) {
...
app.post('/fcm-subscribe', async (req, res) => {
  const { token } = req.body;
  if (!token) return res.status(400).json({ success: false, message: 'Token missing' });

  try {
    await FCMToken.findOneAndUpdate({ token }, {}, { upsert: true });
    res.json({ success: true });
  } catch (err) {
